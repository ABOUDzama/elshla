  ...
  Future<void> _generateAiQuestions() async {
    ...
    try {
      ...
      setState(() {
        ...
        gameStarted = true;
        currentQuestionIndex = gameQuestions.length - newQuestions.length;
        showAnswer = false;
      });
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _forceUpdateData() async {
    ...
    if (mounted) {
      if (result == 'success' || result == 'already_synced') {
        ...
        await _loadGameData(); // Reload data into state
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
        ...
        setState(() {
          isLoadingData = false; // Stop loading if failed
        });
      }
    }
  }
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      ...scoreService.players.asMap().entries.map(
                        (e) => Padding(
